...
class Pixiv
{
    ...
    public function __construct(public null | Origin $origin = null)
    {
    ...
    public function detail(string $id): Collection
    {
        $this->token = $this->refresh();

        $url = "{$this->url}illust/detail?illust_id={$id}";
        return collect(Http::withToken($this->token)->get($url)->collect()['illust'] ?? null);
    }

    public function ugoira(string $id): Ugoira
    {
        $this->token = $this->refresh();

        $url = "{$this->url}ugoira/metadata?illust_id={$id}";

        return Ugoira::make()->metadata(
            Http::withToken($this->token)->get($url)->collect()['ugoira_metadata'] ?? []
        );
    }
}
